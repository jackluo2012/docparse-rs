...
**输出/输入**：`-f/--format`、`--table-format tab\|markdown`、`--chunk-target-chars <n>`（RAG 切块目标字符数，默认 800，仅 `-f chunks` 生效，缺省输出字节不变）、`--password <pw>`（加密 PDF，标准安全处理器 RC4/AES-128/AES-256；无密码加载加密 PDF 报明确错误）、`-o/--out`、位置参数 `inputs`（文件/文件夹/多输入）。
